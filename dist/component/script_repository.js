 ...
 * 打开脚本编辑器
 */
async function onScriptEditorOpenClick(type = 'global', scriptId = null) {
 ...
 * 保存脚本
 */
async function saveScript(script, isNew = false, type = 'global') {
    if (!script.name) {
        toastr.error('保存失败，脚本名称为空');
        return;
    }
    if (isNew) {
        const $scriptItem = $(await renderExtensionTemplateAsync(`${extensionFolderPath}`, 'script_item_template', {
 ...
 *  @param script 脚本内容
 * */
async function saveScriptToExtensionData(scriptArray) {
 ...
 *  @param script 脚本内容
 * */
async function saveScriptToCharacterCard(scriptArray) {
...
export async function loadScriptLibrary() {
    ...
    const baseTemplate = $(await renderExtensionTemplateAsync(`${extensionFolderPath}`, 'script_item_template', {
        scriptName: '',
        id: '',
        moveTo: '',
        faIcon: '',
...
//# sourceMappingURL=script_repository.js.map