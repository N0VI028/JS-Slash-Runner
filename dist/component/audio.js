...
let isAudioEnabled;
export let list_BGMS = [];
export let list_ambients = [];
let bgmEnded = true;
 ...
 */
export function getAudioUrlWithCacheBusting(originalUrl) {
    if (!originalUrl)
        return '';
 ...
 */
export function getNextFileByMode(mode, playlist, currentFile) {
 ...
 * @param status 是否启用
 */
export function toggleAudioControls(type, status = 'enable') {
 ...
 * @param type 音频类型 "bgm" 或 "ambient"
 */
export function initAudioEventListeners(type) {
 ...
 * 点击音频总开关时的处理函数
 */
async function onEnabledClick(userInput = true, enable = true) {
    if (userInput) {
 ...
 * @param type 音频类型 "bgm" 或 "ambient"
 */
export async function playAudio(type) {
...
async function onAudioCooldownInput() {
    extension_settings[extensionName].audio.audio_cooldown = ~~$(`#audio_cooldown`).val();
    saveSettingsDebounced();
}
/**
 * 音频音量变化时的通用处理函数
...
async function onAudioVolumeChange(type) {
    extension_settings[extensionName].audio[`${type}_volume`] = ~~$(`#audio_${type}_volume_slider`).val();
 ...
 * @param type 音频类型 "bgm" 或 "ambient"
 */
export async function togglePlayPause(type) {
    if (!extension_settings[extensionName].audio.audio_setting) {
        return;
    }
    const audioElement = $(`#audio_${type}`)[0];
...
export function initAudioComponents() {
    ...
    $('#audio-enable-toggle')
        ...
        .on('click', (event) => onEnabledClick(true, event.target.checked));
    // 初始化音乐和音效样式
    initAudioStyles('bgm');
