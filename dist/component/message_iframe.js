 ...
 * @param mesId 消息ID
 */
export const renderPartialIframes = (mesId) => {
 ...
 */
export async function clearAllIframes() {
 ...
 * @param mesId 消息ID
 */
function addCodeToggleButtons(mesId) {
    const $chat = $('#chat');
    if (!$chat.length) {
        return;
    }
 ...
 */
async function handleRenderingOptimizationToggle(userInput = true, enable = true) {
    if (userInput) {
 ...
 */
async function handleRenderEnableToggle(userInput, enable) {
    if (userInput) {
        await saveSettingValue('render.render_enabled', enable);
...
export const initIframePanel = () => {
    ...
    $('#render-enable-toggle')
        ...
        .on('click', (event) => handleRenderEnableToggle(true, event.target.checked));
    ...
    $('#render-optimize-toggle')
        ...
        .on('click', (event) => handleRenderingOptimizationToggle(true, event.target.checked));
    // 处理油猴兼容性设置
...
//# sourceMappingURL=message_iframe.js.map