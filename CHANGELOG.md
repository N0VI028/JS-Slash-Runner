...
- 为 `createChatMessages`、`deleteChatMessages`、`rotateChatMessages` 新增与 `setChatMessages` 相同的 `refresh: 'affected'` 选项**并作为默认情况**, 它会使得这些函数只刷新受到影响的楼层 (体验和你在酒馆发送一条消息一样), 而不是刷新整个聊天 (体验和你在酒馆开关正则一样)——**简单来说, 观感和玩家体验上更流畅了**
