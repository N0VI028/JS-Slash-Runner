...
function loadSettings() {
    ...
    $("#enable_ambient").prop("checked", extension_settings[extensionName].audio.ambient_enabled);
    if (!extension_settings[extensionName].audio.ambient_enabled) {
        $("#audio_ambient")[0].pause();
    }
    $("#audio_bgm_volume").text(extension_settings[extensionName].audio.bgm_volume);
    $("#audio_ambient_volume").text(extension_settings[extensionName].audio.ambient_volume);
    $("#audio_bgm_volume_slider").val(extension_settings[extensionName].audio.bgm_volume);
    $("#audio_ambient_volume_slider").val(extension_settings[extensionName].audio.ambient_volume);
    ...
    const ambientMode = extension_settings[extensionName].audio.ambient_mode || "cancel";
    ...
    $("#audio_bgm_cooldown").val(extension_settings[extensionName].audio.bgm_cooldown);
}
...
async function renderMessagesInIframes(mode = RENDER_MODES.FULL, specificMesId = null) {
    ...
    let messagesToCancelIds = [...Array(totalMessages).keys()].filter((id) => !depthLimitedMessageIds.includes(id));
...
async function executeCommand(command) {
    const context = getContext();
    try {
        const result = context.executeSlashCommandsWithOptions(command, true, null, false, null, null, null);
        return result;
    }
    catch (error) {
...
async function onBGMModeClick() {
    ...
    const currentModeIndex = modes.findIndex((m) => m.mode === extension_settings[extensionName].audio.bgm_mode);
...
async function onAmbientModeClick() {
    ...
    const currentModeIndex = modes.findIndex((m) => m.mode === extension_settings[extensionName].audio.ambient_mode);
    const nextModeIndex = (currentModeIndex + 1) % modes.length;
...
async function onBGMVolumeChange() {
    extension_settings[extensionName].audio.bgm_volume = ~~$("#audio_bgm_volume_slider").val();
    $("#audio_bgm").prop("volume", extension_settings[extensionName].audio.bgm_volume * 0.01);
    $("#audio_bgm_volume").text(extension_settings[extensionName].audio.bgm_volume);
    saveSettingsDebounced();
}
async function onAmbientVolumeChange() {
    extension_settings[extensionName].audio.ambient_volume = ~~$("#audio_ambient_volume_slider").val();
    $("#audio_ambient").prop("volume", extension_settings[extensionName].audio.ambient_volume * 0.01);
    $("#audio_ambient_volume").text(extension_settings[extensionName].audio.ambient_volume);
    saveSettingsDebounced();
}
...
async function onAmbientSelectChange() {
    extension_settings[extensionName].audio.ambient_selected = $("#audio_ambient_select").val();
    updateAmbient(true);
    saveSettingsDebounced();
}
async function onBGMCooldownInput() {
    extension_settings[extensionName].audio.bgm_cooldown = ~~$("#audio_bgm_cooldown").val();
    saveSettingsDebounced();
}
...
function updateBGMSelect() {
    ...
    if (list_BGMS && list_BGMS.length > 0) {
        if (!list_BGMS.includes(extension_settings[extensionName].audio.bgm_selected)) {
...
function updateAmbientSelect() {
    ...
    if (list_ambients && list_ambients.length > 0) {
        if (!list_ambients.includes(extension_settings[extensionName].audio.ambient_selected)) {
...
async function updateBGM(isUserInput = false, newChat = false) {
    ...
    if (isUserInput) {
        ...
        audio_url = getNextFileByMode(extension_settings[extensionName].audio.bgm_mode, playlist, extension_settings[extensionName].audio.bgm_selected);
    }
    if (!audio_url) {
        return;
    }
    const audio = $("#audio_bgm")[0];
    if (decodeURIComponent(audio.src) === decodeURIComponent(audio_url) &&
        !bgmEnded) {
        return;
    }
    bgmEnded = false;
...
async function updateAmbient(isUserInput = false) {
    ...
    if (isUserInput) {
        ...
        audio_url = getNextFileByMode(extension_settings[extensionName].audio.ambient_mode, playlist, extension_settings[extensionName].audio.ambient_selected);
    }
    if (!audio_url) {
        return;
    }
    ...
    if (decodeURIComponent(cleanAudioSrc) === decodeURIComponent(cleanAudioUrl) &&
        !ambientEnded) {
        return;
    }
    ambientEnded = false;
...
jQuery(async () => {
  ...
  <div id="js_slash_runner_container" class="list-group-item flex-container flexGap5 interactable">
    ...
    $("#tampermonkey_compatibility").on("click", onTampermonkeyCompatibilityChange);
    ...
    SlashCommandParser.addCommandObject(SlashCommand.fromProps({
        ...
        namedArgumentList: [
            ...
            new SlashCommandNamedArgument("play", "播放或暂停", [ARGUMENT_TYPE.STRING], true, false, "true", commonEnumProviders.boolean("trueFalse")()),
        ],
        helpString: `
        <div>
            控制音乐播放器或音效播放器的播放与暂停。
    ...
    SlashCommandParser.addCommandObject(SlashCommand.fromProps({
        ...
        namedArgumentList: [
            ...
            new SlashCommandNamedArgument("state", "打开或关闭播放器", [ARGUMENT_TYPE.STRING], false, false, "true", commonEnumProviders.boolean("trueFalse")()),
        ],
        helpString: `
        <div>
            控制音乐播放器或音效播放器的开启与关闭。
