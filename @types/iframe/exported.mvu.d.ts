...
declare const Mvu: {
  events: {
     ...
     *   const delta = _.get(variables, 'stat_data.角色.络络.好感度') - _.get(variables_before_update, 'stat_data.角色.络络.好感度');
     *   if (Math.abs(delta) > 3) {
     *     _.set(variables, 'stat_data.角色.络络.好感度', _.get(variables_before_update, 'stat_data.角色.络络.好感度') + Math.sign(delta) * 3);
     *   }
     * });
     */
