 ...
 *   - `generation_id?:string`: 生成请求唯一标识符
 *   - `concurrency_group?:'default'|'unique'|'bypass'|string`: 并发组设置
 *   - `preset_name?:'in_use'|string`: 要使用的预设名称
 *   - `should_stream?:boolean`: 是否启用流式传输; 默认为 'false'
 *   - `should_silence?:boolean`: 是否静默生成; 默认为 'false'
 *   - `image?:File|string|(File|string)[]`: 图片输入, 支持单张或多张图片
 ...
 *   - `json_schema?:JsonSchema`: JSON Schema 定义
 * @returns 生成的最终文本, 或工具调用结果 `GenerateToolCallResult`
 *
 * @example
 * // 请求生成
 ...
 *   - `injects?:Omit<InjectionPrompt, 'id'>[]`: 要额外注入的提示词
 *   - `max_chat_history?:'all'|number`: 最多使用多少条聊天历史
 *   - `ordered_prompts?:(PlaceholderPrompt|RolePrompt)[]`: 一个提示词数组, 数组元素将会按顺序发给 AI, 因而相当于自定义预设
 *   - `custom_api?:CustomApiConfig`: 自定义API配置
 *   - `tools?:ToolDefinition[]`: 工具定义列表
 *   - `tool_choice?:ToolChoice`: 工具选择策略
 *   - `json_schema?:JsonSchema`: JSON Schema 定义
...
type GenerateConfig = {
  ...
  /**
   * 并发组, 同一并发组的 `generate`/`generateRaw` 请求才能并发执行, 否则需要排队等待; 默认为 'default' 并发组
   * - 'default': 默认并发组
   * - 'unique': 为这个请求单独设置一个并发组, 与其他所有请求互斥, 不能同时进行
   * - 'bypass': 特殊情况, 将会忽略并发组设置, 可以与任何请求同时进行
   * - string: 自定义并发组; 例如 '变量解析'
   *
   * @example
   * // 两个 'A' 组的请求能同时生成; 'B' 则需要排队等待 'A' 生成结束; 'bypass' 绕过并发要求因此能和 'A' 或 'B' 同时生成
   * const generateA1     = generate({ user_input: 'A1', concurrency_group: 'A' });
   * const generateA2     = generateRaw({ user_input: 'A2', concurrency_group: 'A' });
   * const generateB      = generate({ user_input: 'B', concurrency_group: 'B' });
   * const generateBypass = generateRaw({ user_input: '绕过并发组', concurrency_group: 'bypass' });
