<template>
  <div class="relative mb-1 flex flex-col gap-0.25 overflow-hidden rounded-sm bg-(--SmartThemeChatTintColor) p-0.5">
    <div class="flex items-center justify-between">
      <div class="flex flex-grow items-center gap-0.5 overflow-hidden">
        <i></i>
        <input
          type="text"
          class="w-full flex-grow border-none bg-transparent text-sm font-bold text-(--SmartThemeBodyColor)"
          value="{{name}}"
        />
      </div>
      <div class="flex flex-shrink-0 flex-col items-end gap-0.5">
        <div class="cursor-pointer border-none bg-none p-0.5" title="{{t`保存`}}">
          <i class="fa-regular fa-save"></i>
        </div>
        <div class="cursor-pointer border-none bg-none p-0.5" title="{{t`删除`}}">
...
.variable-card[data-type='object'] {
  padding: 5px;
}
...
textarea.variable-content-input {
  height: calc(var(--mainFontSize) * 2);
  line-height: calc(var(--mainFontSize) * 1.34);
}
...
#variable-manager-container .list-item {
  display: flex;
  gap: 5px;
  align-items: center;
}
...
.nested-cards-container .variable-card[data-type='string'] {
  border-left: 2px solid var(--vm-type-string) !important;
}
.nested-cards-container .variable-card[data-type='number'] {
  border-left: 2px solid var(--vm-type-number) !important;
}
.nested-cards-container .variable-card[data-type='boolean'] {
  border-left: 2px solid var(--vm-type-boolean) !important;
}
.nested-cards-container .variable-card[data-type='array'] {
  border-left: 2px solid var(--vm-type-array) !important;
}
.nested-cards-container .variable-card[data-type='object'] {
  border-left: 2px solid var(--vm-type-object) !important;
}
