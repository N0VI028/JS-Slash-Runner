...
const props = defineProps<{ id: string; name: string; content: string; useBlobUrl: boolean; useCleanupHelper: boolean }>();
...
const src_prop = computed((old_src_prop?: { srcdoc?: string; src?: string }) => {
  ...
  const content = createSrcContent(props.content, props.useBlobUrl, props.useCleanupHelper);
  if (!props.useBlobUrl) {
