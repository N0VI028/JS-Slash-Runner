...
export function useJQueryDrag() {
  async function reorderFolderByDom(folderEl: HTMLElement, repoType: ScriptType): Promise<boolean> {
    ...
    const orderedIds: string[] = $folderContent
      ...
      .map((_, el) => (el as HTMLElement).id)
      .get();

    try {
      await repositoryService.reorderFolderScripts(repoType, folderId, orderedIds);
      return true;
    } catch (error) {
      console.error('[拖拽] 文件夹重排失败:', error);
      return false;
    }
  }

  async function reorderRootByDom(containerEl: HTMLElement, repoType: ScriptType): Promise<boolean> {
    const $container = $(containerEl);
    const orderedEls = $container.children('.script-item, .script-folder');
    const orderedIds: string[] = orderedEls.map((_, el) => (el as HTMLElement).id).get();

    try {
      await repositoryService.reorderRepositoryItems(repoType, orderedIds);
      return true;
    } catch (error) {
      console.error('[拖拽] 根列表重排失败:', error);
      return false;
    }
  }

  // 脚本列表sortable - 使用watchEffect监听元素变化
  const useListSortable = (el: Ref<HTMLElement | undefined>, repoType: ScriptType) => {
    ...
    const stopWatcher = watchEffect(() => {
      ...
      try {
        $element.sortable({
          ...
          stop: async () => {
            try {
              const applied = await reorderRootByDom($element.get(0)!, repoType);
              if (!applied) $element.sortable('cancel');
            } catch (error) {
              console.error('[拖拽] 根列表排序失败:', error);
  ...
  // 文件夹内容sortable
  const useFolderSortable = (el: Ref<HTMLElement | undefined>, repoType: ScriptType) => {
    const stopWatcher = watchEffect(() => {
      if (!el.value) return;
      ...
      $folderContent.sortable({
        ...
        stop: async () => {
          try {
            const applied = await reorderFolderByDom(el.value!, repoType);
            if (!applied) $folderContent.sortable('cancel');
          } catch (error) {
            console.error('[拖拽] 文件夹内排序失败:', error);
            $folderContent.sortable('cancel');
          }
        },
      });
