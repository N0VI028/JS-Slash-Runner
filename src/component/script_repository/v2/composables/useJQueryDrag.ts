...
export function useJQueryDrag() {
  ...
  const useRootDrop = (el: Ref<HTMLElement | undefined>, repoType: ScriptType) => {
    const stopWatcher = watchEffect(() => {
      ...
      $element.on('dragover.dragdrop-root dragenter.dragdrop-root dragleave.dragdrop-root drop.dragdrop-root', function (e) {
        e.preventDefault();
        e.stopPropagation();
        return false;
      });
      ...
      $element.droppable({
        ...
        drop: async (event, ui) => {
          ...
          try {
            if (repoType === 'global') {
              await repositoryService.moveGlobalScript(scriptId, null);
            } else {
              await repositoryService.moveCharacterScript(scriptId, null);
            }
            await refreshData();
  ...
  const useScriptElement = (el: Ref<HTMLElement | undefined>, scriptId: string) => {
    onMounted(() => nextTick(() => {
      ...
      $(el.value)
        .attr({ id: scriptId, 'data-script-repository': 'true' })
        .addClass('script-item');
    }));
  };
