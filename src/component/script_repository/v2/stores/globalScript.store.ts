...
export const useGlobalScriptStore = defineStore('globalScript', () => {
  ...
  async function loadRepository(): Promise<void> {
    try {
      ...
      const data = await repositoryService.loadGlobalRepository();
  ...
  async function saveRepository(): Promise<void> {
    try {
      await repositoryService.saveGlobalRepository(repository.value);
    } catch (err) {
  ...
  async function createScript(payload: CreateScriptPayload): Promise<string> {
    try {
      error.value = null;
      const scriptId = await repositoryService.createGlobalScript(payload);
  ...
  async function updateScript(scriptId: string, payload: UpdateScriptPayload): Promise<void> {
    try {
      error.value = null;
      await repositoryService.updateGlobalScript(scriptId, payload);
  ...
  async function deleteScript(scriptId: string): Promise<void> {
    try {
      error.value = null;
      await repositoryService.deleteGlobalScript(scriptId);
  ...
  async function createFolder(payload: CreateFolderPayload): Promise<string> {
    try {
      error.value = null;
      const folderId = await repositoryService.createGlobalFolder(payload);
  ...
  async function deleteFolder(folderId: string): Promise<void> {
    try {
      error.value = null;
      await repositoryService.deleteGlobalFolder(folderId);
  ...
  async function moveScript(scriptId: string, targetFolderId: string | null): Promise<void> {
    try {
      error.value = null;
      await repositoryService.moveGlobalScript(scriptId, targetFolderId);
  ...
  async function toggleScriptEnabled(scriptId: string): Promise<void> {
    ...
    try {
      ...
      log.info(`[ScriptRepository] 脚本[${script.name}]已${!script.enabled ? '启用' : '禁用'}成功`);

    } catch (err) {
      error.value = err instanceof Error ? err.message : '切换脚本启用状态失败';
      throw err;
    }
